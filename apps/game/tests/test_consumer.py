...
async def _connect_and_join(**join_payload):
...
async def test_player_join_receives_confirmation(lobby_session):
    player = await _connect_and_join(role='player', code=lobby_session.code, nickname='jugador1')
...
async def test_join_unknown_code_returns_error(lobby_session):
    player = await _connect_and_join(role='player', code='NOEXISTE', nickname='jugador1')
...
async def test_host_join_with_wrong_token_returns_error(lobby_session):
    host = await _connect_and_join(role='host', code=lobby_session.code, host_token='wrong-token')
...
async def test_rejoin_with_same_nickname_reconnects_to_existing_player(lobby_session):
    first = await _connect_and_join(role='player', code=lobby_session.code, nickname='jugador1')
    first_ack = json.loads(await first.receive_from())
    await first.disconnect()

    second = await _connect_and_join(role='player', code=lobby_session.code, nickname='jugador1')
...
async def test_full_game_flow_between_host_and_player(lobby_session):
    host = await _connect_and_join(role='host', code=lobby_session.code, host_token=lobby_session.host_token)
...
async def test_end_before_start_returns_error(lobby_session):
    host = await _connect_and_join(role='host', code=lobby_session.code, host_token=lobby_session.host_token)
